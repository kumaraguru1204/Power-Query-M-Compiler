...
#[test]
fn t02_zero_rows() {
    assert!(run_query(r#"Table.LastN(Source, 0)"#).is_ok());
}
...
fn t03_expression_count() {
    assert!(run_query(r#"Table.LastN(Source, 1 + 1)"#).is_ok());
}

#[test]
fn t04_expression_variable() {
    assert!(run_query(r#"
        let
            n = 2
        in
            Table.LastN(Source, n)
    "#).is_ok());
}

// ── Gap 1 — predicate (take-from-bottom) ──────────────────────────────────

#[test]
fn t05_each_predicate() {
    assert!(run_query(r#"
        Table.LastN(Source, each _[Age] <> "30")
    "#).is_ok());
}
...
#[test]
fn t09_nested_transform_columns() {
    assert!(run_query(r#"
        Table.LastN(
...
// ── Gap 4 — SQL generator (predicate form) ────────────────────────────────

#[test]
fn t10_predicate_simple() {
    assert!(run_query(r#"
        let
            Source = Excel.Workbook(File.Contents("dummy.xlsx"), null, true)
        in
            Table.LastN(Source, each _[Name] <> "Alice")
    "#).is_ok());
}
...
fn t11_predicate_with_and() {
    assert!(run_query(r#"
        let
            Source = Excel.Workbook(File.Contents("dummy.xlsx"), null, true)
        in
            Table.LastN(Source, each _[Age] <> "25" and _[Name] <> "Bob")
    "#).is_ok());
}

#[test]
fn t12_predicate_field_access() {
    assert!(run_query(r#"
        Table.LastN(Source, each _[Age] <> "28")
    "#).is_ok());
}
...
#[test]
fn t13_large_n() {
    assert!(run_query(r#"Table.LastN(Source, 100)"#).is_ok());
}
