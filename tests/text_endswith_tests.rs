...
#[test]
fn t01_basic_true() {
    assert!(run_query(r#"Text.EndsWith("Apple","e")"#).is_ok());
}

#[test]
fn t02_basic_false() {
    assert!(run_query(r#"Text.EndsWith("Apple","A")"#).is_ok());
}
...
fn t05_substring_from_function() {
    assert!(run_query(r#"
        Text.EndsWith("Apple", Text.End("ABCDE",1))
    "#).is_ok());
}
...
fn t06_with_ordinal_ignore_case() {
    assert!(run_query(r#"
        Text.EndsWith("Apple", "E", Comparer.OrdinalIgnoreCase)
    "#).is_ok());
}
...
fn t08_with_options_record() {
    assert!(run_query(r#"
        Text.EndsWith("Apple", "E", [Comparer = Comparer.OrdinalIgnoreCase])
    "#).is_ok());
}
...
#[test]
fn t15_empty_string() {
    assert!(run_query(r#"Text.EndsWith("", "e")"#).is_ok());
}

#[test]
fn t16_empty_substring() {
    assert!(run_query(r#"Text.EndsWith("Apple", "")"#).is_ok());
}

#[test]
fn t17_both_empty() {
    assert!(run_query(r#"Text.EndsWith("", "")"#).is_ok());
}
